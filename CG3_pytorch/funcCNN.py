...
def arr2sparse(arr):
    """Return a torch sparse_coo_tensor from a dense numpy array."""
    arr_idx = np.argwhere(arr != 0)
    indices = torch.from_numpy(arr_idx.T.astype('int64'))
    values = torch.from_numpy(arr[arr_idx[:, 0], arr_idx[:, 1]].astype('float32'))
    shape = torch.Size(np.shape(arr))
...
def construct_feed_dict_1(support, features, labels, labels_mask, placeholders, m1, l1):
    """Construct feed dictionary (used to update the placeholders dict each step)."""
    feed_dict = dict()
